...
public:
	...
	bool kontrol(int p, int i, int j);
};
Sudoku oyun;
...
bool Sudoku::kontrol(int p, int i, int j)
{
	for (int k = 1; k <= 9; k++)
...
void Sudoku::cozum(int i, int j)
{
	...
	if (kontrol[i][j] == 1) return oyun.cozum(i, j - 1);
	for (int p = 1; p <= 9; p++)
		if (kontrol(p, i, j))
		{
			board[i][j] = p;
	...
	return oyun.cozum(i, j - 1);

}
void Sudoku::sudoku_coz()
{
	for (int i = 1; i <= 9; i++)
		for (int j = 1; j <= 9; j++)
			if (board[i][j] == 0) oyun.cozum(i, j);
